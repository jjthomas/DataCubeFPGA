    ...
    "Test1" -> { (backendName: String) =>
      Driver(() => new StreamingWrapper(0, 1000000000, 64, 1,
        ...
        (c) => {
          ...
          // The second row is unused since the stream length is one.
          // The two data streams to be crossed are identical here.
          ...
          new StreamingWrapperTests(c, Util.arrToBits(Array(1, 7), 64), Util.arrToBits(Array(1, 7), 64),
